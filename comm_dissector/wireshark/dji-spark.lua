...
-- [4]  Sender
f.sender = ProtoField.uint8 ("dji_spark.sender", "Sender", base.DEC, DJI_SPARK_FLIGHT_CONTROL_UART_SRC_DEST, 0x1F)
-- [5]  Receiver
f.receiver = ProtoField.uint8 ("dji_spark.receiver", "Receiver", base.DEC, DJI_SPARK_FLIGHT_CONTROL_UART_SRC_DEST, 0x1F)
-- [6-7]  Sequence Ctr
f.seqctr = ProtoField.uint16 ("dji_spark.seqctr", "Seq Counter", base.DEC)
-- [8] Encryption
f.encrypt = ProtoField.uint8 ("dji_spark.encrypt", "Encryption Type", base.DEC, DJI_SPARK_FLIGHT_CONTROL_UART_ENCRYPT_TYPE, 0x0F)
-- [8] Ack (to be improved)
f.ack = ProtoField.uint8 ("dji_spark.ack", "Ack", base.HEX, DJI_SPARK_FLIGHT_CONTROL_UART_ACK_POLL, 0x60)
-- [9] Cmd Set
f.cmdset = ProtoField.uint8("dji_spark.cmdset", "Cmd Set", base.DEC, DJI_SPARK_FLIGHT_CONTROL_UART_CMD_SET, 0xFF)
-- [A] Cmd
...
local function main_dissector(buffer, pinfo, subtree)
    ...
    if pkt_protover == 0 then
        ...
        local valuestring = DJI_SPARK_FLIGHT_CONTROL_UART_CMD_TYPE[cmdset] or {}
        subtree:add (f.cmd, buffer(offset, 1), cmd, string.format("%s: %s (0x%02X)", "Cmd", valuestring[cmd] or "Unknown", cmd))
        offset = offset + 1

        set_info(cmd, pinfo, DJI_SPARK_FLIGHT_CONTROL_UART_CMD_TYPE[cmdset])
        ...
        if pkt_length > offset+2 then
            ...
            -- If we have a dissector for this kind of command, run it
            local dissector_group = DJI_SPARK_FLIGHT_CONTROL_UART_DISSECT[cmdset] or {}
